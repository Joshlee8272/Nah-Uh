...
local seedsFrame = player:WaitForChild("PlayerGui"):WaitForChild("Main"):WaitForChild("Seeds"):WaitForChild("Frame"):WaitForChild("ScrollingFrame")
local gearsFrame = player:WaitForChild("PlayerGui"):WaitForChild("Main"):WaitForChild("Gears"):WaitForChild("Frame"):WaitForChild("ScrollingFrame")
...
-- Extract stock number from text like "x10"
local function getStock(text)
    local amount = text:match("x(%d+)")
    return tonumber(amount) or 0
end

-- Get list of item names from a scrolling frame
...
-- Buy item by firing remote event multiple times
local function buyItem(itemName, amount, isSeed)
    local arg2 = isSeed and "\b" or "\026" -- These seem to be special codes for seeds/gears
    for i = 1, amount do
        dataRemoteEvent:FireServer({itemName, arg2})
    end
end
...
if not tierModel then
    warn("Tier model not found in plot")
    return
end

local roadModel = tierModel:FindFirstChild("Road")
if not roadModel then
    warn("Road model not found in tier model")
    return
end
...
local function clip()
    ...
    -- Restore CanCollide to true for character parts (optional)
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "plants vs. brainrot",
